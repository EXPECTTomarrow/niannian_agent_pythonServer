...
def import_draft_skill() -> SkillRegistry:
    ...
    registry.register(Tool("import.mutate", "Change only the current client-side import draft. Use delete with a filter, or update with one target and one editable field. Never writes contacts.", mutate, parameters={"type": "object", "properties": {"action": {"type": "string", "enum": ["delete", "update"]}, "filter": filter_schema["properties"]["filter"], "target": {"type": "object", "properties": {"sourceRow": {"type": "integer"}, "name": {"type": "string"}, "all": {"type": "boolean"}}}, "changes": {"type": "object", "properties": {"field": {"type": "string", "enum": ["name", "birthday", "gender", "phone", "address", "company", "relation", "relationNote", "interests", "skills", "dislikes", "education", "giftNote", "note", "importance"]}, "value": {"type": "string"}}}}}))
